  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden bg-[#030305]">
      <style>{`
        ...
        @keyframes monitorDraw { 0% { -webkit-mask-position: 200% 0; opacity: 1; } 60% { -webkit-mask-position: -100% 0; opacity: 1; } 61% { opacity: 0; } 100% { -webkit-mask-position: 200% 0; opacity: 0; } }
        .ecg-line { -webkit-mask-image: linear-gradient(to right, transparent 0%, rgba(0,0,0,0.1) 60%, rgba(0,0,0,1) 95%, transparent 100%); -webkit-mask-size: 200% 100%; -webkit-mask-repeat: no-repeat; animation: monitorDraw 5s linear infinite; }
      `}</style>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSensorData({
        ...
        motorTorque: (((-Math.cos(t) + 1) / 2) * 8.5).toFixed(1),
        actuatorLoad: Math.floor(((Math.sin(t) + 1) / 2) * 100),
      });
    }, 50); 
  ...
  const getDynamicColor = (val, max) => {
    const ratio = val / max;
    if (ratio < 0.30) return { text: "text-emerald-400 drop-shadow-[0_0_15px_rgba(52,211,153,0.8)]", bg: "bg-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.8)]" };          
    if (ratio < 0.60) return { text: "text-teal-400 drop-shadow-[0_0_15px_rgba(45,212,191,0.8)]", bg: "bg-teal-400 shadow-[0_0_15px_rgba(45,212,191,0.8)]" }; 
    if (ratio < 0.85) return { text: "text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.8)]", bg: "bg-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.8)]" };
    return { text: "text-red-500 drop-shadow-[0_0_15px_rgba(239,68,68,0.8)]", bg: "bg-red-500 shadow-[0_0_15px_rgba(239,68,68,0.8)]" };                        
  };

  const flexColors = getDynamicColor(sensorData.flexionAngle, 120);
  const torqueColors = getDynamicColor(sensorData.motorTorque, 8.5);
  const loadColors = getDynamicColor(sensorData.actuatorLoad, 100);

  return (
    <main 
      onMouseMove={handleMouseMove} 
      className="relative flex min-h-screen flex-col items-center bg-[#030305] text-gray-100 overflow-x-hidden selection:bg-teal-500/30"
    >
      <HomeBackground />

      {/* --- SPOTLIGHT EFFECT --- */}
      <div 
        className="pointer-events-none fixed z-0 transition-transform duration-75 ease-out" 
        style={{
          width: "600px",
          height: "600px",
      ...
      <motion.div id="telemetry" initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }} variants={staggerContainer} className="z-10 w-full max-w-6xl py-20 px-8 relative">
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-teal-600/10 blur-[150px] pointer-events-none z-0" />
        
        <div className="relative z-10">
          <motion.h3 variants={fadeUpVariant} className="text-3xl md:text-4xl font-black mb-2 text-white tracking-tight">Live Telemetry Simulation</motion.h3>
          <motion.p variants={fadeUpVariant} className="text-gray-400 mb-12 font-light">Real-time data streaming capabilities modeled directly from the MPU6050 IMU and control loop.</motion.p>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <motion.div variants={fadeUpVariant} className="bg-[#050505]/95 backdrop-blur-3xl p-8 rounded-[30px] shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-teal-500/20 hover:border-teal-400/60 transition-all duration-500 hover:shadow-[0_0_30px_rgba(45,212,191,0.15)] hover:-translate-y-1">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em] mb-6">Flexion Angle</h4>
              <div className="flex items-end gap-2 mb-6">
                <span className={`text-7xl font-black tracking-tighter ${flexColors.text}`}>{sensorData.flexionAngle}°</span>
              </div>
              <div className="w-full bg-gray-900/80 rounded-full h-2.5 overflow-hidden border border-white/5">
                <div className={`h-full rounded-full ${flexColors.bg}`} style={{ width: `${(sensorData.flexionAngle / 120) * 100}%`, transition: 'width 0.1s linear, background-color 0.3s' }}></div>
              </div>
            </motion.div>

            <motion.div variants={fadeUpVariant} className="bg-[#050505]/95 backdrop-blur-3xl p-8 rounded-[30px] shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-teal-500/20 hover:border-teal-400/60 transition-all duration-500 hover:shadow-[0_0_30px_rgba(45,212,191,0.15)] hover:-translate-y-1">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em] mb-6">Motor Torque Output</h4>
              <div className="flex items-end gap-2 mb-6">
                <span className={`text-7xl font-black tracking-tighter ${torqueColors.text}`}>{sensorData.motorTorque}</span>
                <span className="text-xl font-bold text-gray-500 mb-2 tracking-widest">Nm</span>
              </div>
              <div className="w-full bg-gray-900/80 rounded-full h-2.5 overflow-hidden border border-white/5">
                <div className={`h-full rounded-full ${torqueColors.bg}`} style={{ width: `${(parseFloat(sensorData.motorTorque) / 8.5) * 100}%`, transition: 'width 0.1s linear, background-color 0.3s' }}></div>
              </div>
            </motion.div>

            <motion.div variants={fadeUpVariant} className="bg-[#050505]/95 backdrop-blur-3xl p-8 rounded-[30px] shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-teal-500/20 hover:border-teal-400/60 transition-all duration-500 hover:shadow-[0_0_30px_rgba(45,212,191,0.15)] hover:-translate-y-1">
              <h4 className="text-xs font-bold text-gray-400 uppercase tracking-[0.2em] mb-6">Actuator Load</h4>
              <div className="flex items-end gap-2 mb-6">
                <span className={`text-7xl font-black tracking-tighter ${loadColors.text}`}>{sensorData.actuatorLoad}%</span>
              </div>
              <div className="w-full bg-gray-900/80 rounded-full h-2.5 overflow-hidden border border-white/5">
                <div className={`h-full rounded-full ${loadColors.bg}`} style={{ width: `${sensorData.actuatorLoad}%`, transition: 'width 0.1s linear, background-color 0.3s' }}></div>
              </div>
            </motion.div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-16 xl:px-8">
            ...
            <div>
              ...
              <ul className="space-y-4 text-sm text-gray-400">
                <li><button onClick={scrollToTop} className="hover:text-teal-400 transition-colors text-left">Home</button></li>
