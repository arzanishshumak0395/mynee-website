...
// ==========================================
// INTERSTELLAR ANIMATIONS & EFFECTS
// ==========================================
const fadeUp = { 
...
// New: Slow-Mo Interstellar Starfield
const SlowMoStars = () => {
  const [stars, setStars] = useState([]);
  
  useEffect(() => {
    // Generate static stars once on mount to avoid hydration mismatch
    setStars([...Array(120)].map(() => ({
      size: Math.random() * 2.5 + 0.5,
      left: `${Math.random() * 100}%`,
      top: `${Math.random() * 100}%`,
      duration: 30 + Math.random() * 40, // Super slow (30s - 70s)
    ...
    <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
      {stars.map((star, i) => (
        <motion.div
          ...
          animate={{
            y: [-20, -100], // Drift slowly upwards
            opacity: [star.opacity, star.opacity * 2, 0], // Pulse gently then fade out
          }}
          transition={{
  ...
  return (
    <main className="relative flex min-h-screen flex-col bg-[#030305] text-gray-100 font-sans selection:bg-yellow-500 selection:text-black overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute inset-0 bg-[linear-gradient(to_bottom,rgba(0,0,0,0)_0%,rgba(0,0,0,0.8)_100%)] z-10" />
        
        {/* Slow rotating nebulas */}
        <motion.div 
          animate={{ rotate: 360, scale: [1, 1.1, 1] }} 
          transition={{ duration: 150, repeat: Infinity, ease: "linear" }} 
          className="absolute top-[-20%] left-[-10%] w-[60vw] h-[60vw] bg-indigo-900/20 blur-[150px] rounded-full mix-blend-screen"
        />
        <motion.div 
          animate={{ rotate: -360, scale: [1, 1.2, 1] }} 
          transition={{ duration: 180, repeat: Infinity, ease: "linear" }} 
          className="absolute bottom-[-20%] right-[-10%] w-[70vw] h-[70vw] bg-yellow-600/10 blur-[150px] rounded-full mix-blend-screen"
        />
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto px-6 pt-48 pb-32 flex flex-col items-center flex-1">
        ...
        <motion.div 
          ...
          className="w-full bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[40px] p-8 md:p-16 shadow-[0_0_40px_rgba(0,0,0,0.5)] grid grid-cols-1 lg:grid-cols-12 gap-16 relative overflow-hidden"
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="group relative">
                <input 
                  ...
                  className="peer w-full bg-transparent border-b border-gray-700 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors placeholder-transparent"
                />
              ...
              <div className="group relative pt-2">
                <textarea 
                  ...
                  className="peer w-full bg-transparent border-b border-gray-700 py-3 text-white focus:outline-none focus:border-yellow-500 transition-colors placeholder-transparent resize-none"
                ></textarea>
          ...
          <motion.div variants={fadeUp} className="lg:col-span-5 flex flex-col gap-6 relative z-10 lg:pl-8 lg:border-l border-gray-800">
            ...
            <div className="space-y-4">
              <a href="mailto:syedarzanish@gmail.com" className="group flex items-center justify-between bg-black/40 border border-white/5 hover:border-yellow-500/50 p-4 rounded-2xl transition-all hover:bg-white/5">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-gray-900 border border-white/5 flex items-center justify-center text-gray-400 group-hover:text-yellow-500 transition-colors">
                    <MailIcon />
                  </div>
              ...
              <div className="group flex items-center justify-between bg-black/40 border border-white/5 p-4 rounded-2xl cursor-default">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-gray-900 border border-white/5 flex items-center justify-center text-gray-400">
                    <UniversityIcon />
                  </div>
            ...
            <div className="mt-auto pt-8 border-t border-gray-800">
              <p className="text-gray-400 text-xs leading-relaxed font-light">
                <strong className="text-gray-200 block mb-1">Project Status: Complete</strong>
      ...
      {/* --- MEGA FOOTER --- */}
      <footer className="relative z-10 w-full bg-black/80 backdrop-blur-xl border-t border-white/5 pt-20 pb-10 mt-auto">
        <div className="relative z-10 max-w-6xl mx-auto px-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
            <div>
              <div className="text-xl font-black text-yellow-500 mb-2 tracking-tighter">MYNEE</div>
              <p className="text-gray-500 text-xs">Advanced Edge-Computed Biomechanics.</p>
            </div>
            <div className="flex gap-6 text-xs font-bold uppercase tracking-widest text-gray-400">
              <Link href="/" className="hover:text-white transition-colors">Home</Link>
              <Link href="/devlog" className="hover:text-white transition-colors">Dev Log</Link>
          ...
          <div className="mt-12 pt-8 border-t border-white/5 text-center text-[10px] text-gray-600 uppercase tracking-widest">
            <p>© 2026 MYNEE | Syed Arzanish | Built with Next.js</p>
          </div>
        </div>
      </footer>
    </main>
